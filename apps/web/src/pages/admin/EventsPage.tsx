...
function CreateEventSheet({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-30 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-3xl max-h-[92dvh] overflow-y-auto px-6 pt-6 pb-8 space-y-3">
  ...
  return (
    <div className="fixed inset-0 z-30 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-3xl max-h-[92dvh] overflow-y-auto px-6 pt-6 pb-8">
        ...
        {event.description && <p className="text-sm text-gray-500 mt-1">{event.description}</p>}
        <p className="text-xs text-gray-400 mt-2">До: {formatDateTime(event.endsAt)}</p>
        <p className="text-xs text-gray-500 mt-1">+{event.rewardPoints} / -{event.penaltyPoints} ⭐</p>
        {isEventActive(event.endsAt) && (
        ...
        <div className="mt-4 space-y-2">
          {event.participants.map((participant) => (
            <div key={participant.squadId} className="rounded-xl border border-gray-200 p-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="font-semibold text-gray-900">{participant.squadName}</p>
                ...
                {participant.status === 'pending' ? (
                  <div className="flex gap-2">
                    <button
                      onClick={() => mutate({ squadId: participant.squadId, status: 'completed' })}
                      disabled={isPending}
                    ...
                    <button
                      onClick={() => mutate({ squadId: participant.squadId, status: 'failed' })}
                      disabled={isPending}
                  ...
                  <span className="text-xs text-gray-400">{participant.resolutionSource ?? '—'}</span>
                )}
              </div>
            </div>
          ))}
