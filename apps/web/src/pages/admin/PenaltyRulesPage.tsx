  ...
  return (
    <div className={`bg-white rounded-2xl px-4 py-3.5 shadow-sm flex items-center gap-3 ${!rule.isActive ? 'opacity-50' : ''}`}>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-red-500 font-semibold mt-0.5">-{Math.abs(rule.points)} балів</p>
      </div>
      <button
        onClick={() => toggle(rule.id)}
        disabled={isPending}
        className={`flex-shrink-0 text-xs font-medium px-3 py-1.5 rounded-xl transition-colors ${
          rule.isActive ? 'bg-gray-100 text-gray-500 active:bg-red-50 active:text-red-500' : 'bg-green-100 text-green-600'
        }`}
      >
...
function CreatePenaltyRuleSheet({ onClose }: { onClose: () => void }) {
  const [label, setLabel] = useState('')
  const [points, setPoints] = useState('')
  const { mutate, isPending, error } = useCreatePenaltyRule()
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input
              ...
              placeholder="Запізнення, порушення тиші..."
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Скільки віднімати балів *</label>
            <input
              type="number"
              min="1"
              value={points}
    ...
    <div className="p-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">Покарання</h1>
          <p className="text-sm text-gray-400 mt-0.5">{active.length} активних правил</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="font-medium">Правил покарання ще немає</p>
          <p className="text-sm mt-1">Натисніть + щоб додати перше правило</p>
        </div>
      ) : (
        <div className="space-y-2">
          {active.map((rule) => <RuleRow key={rule.id} rule={rule} />)}
          {inactive.length > 0 && (
            <>
              <p className="text-xs text-gray-400 uppercase tracking-wider mt-4 mb-2">Вимкнені</p>
              {inactive.map((rule) => <RuleRow key={rule.id} rule={rule} />)}
            </>
          )}
        </div>
      )}

      {showCreate && <CreatePenaltyRuleSheet onClose={() => setShowCreate(false)} />}
    </div>
  )
}
